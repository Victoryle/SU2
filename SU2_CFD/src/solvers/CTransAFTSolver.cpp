...
CTransAFTSolver::CTransAFTSolver(CGeometry *geometry, CConfig *config, unsigned short iMesh)
  ...
  const su2double AF1_Inf = 0.0;
  const su2double AF2_Inf = 0.0;

  Solution_Inf[0] = AF1_Inf;
  Solution_Inf[1] = AF2_Inf;

  /*--- Initialize the solution to the far-field state everywhere. ---*/
  nodes = new CTransAFTVariable(AF1_Inf, AF2_Inf, 1.0, nPoint, nDim, nVar, config);  
  ...
  for (unsigned long iMarker = 0; iMarker < nMarker; iMarker++) {
    ...
    for (unsigned long iVertex = 0; iVertex < nVertex[iMarker]; ++iVertex) {
      Inlet_TurbVars[iMarker](iVertex,0) = AF1_Inf;
      Inlet_TurbVars[iMarker](iVertex,1) = AF2_Inf;
    }
  }
...
void CTransAFTSolver::Postprocessing(CGeometry *geometry, CSolver **solver_container, CConfig *config, unsigned short iMesh) {
  ...
  for (unsigned long iPoint = 0; iPoint < nPoint; iPoint ++) {
    ...
    /*--- Cal Ns ---*/
    su2double delta_MeL = max(Ma_eL - 4, 0.0);
    ...
    const su2double f_lim = exp(1.0 - StrainMag * StrainMag / VorticityMag / VorticityMag);
    ...
    const su2double Alge_gamma_s = min(3.0 * PI_1 * PI_2 * PI_3 * Alge_gamma, 3.0);
  ...
  BEGIN_SU2_OMP_SAFE_GLOBAL_ACCESS {
    ...
    for (auto iPoint_Global = 0ul; iPoint_Global < geometry[MESH_0]->GetGlobal_nPointDomain(); iPoint_Global++) {
      ...
      if (iPoint_Local > -1) {
        ...
        nodes ->SetIntermittencyAlgeEff(iPoint_Local,  Restart_Data[index + 2]);
